...
class RouteDelete : public Php::Base {

    public: RouteDelete(Php::Parameters &param) {
        ...
        if(request_method == "DELETE") {
            ...
            if(!Database::get::boolean({"FUSION_STORE", "FS_ROUTE", "FS_Route_V_Double"})) {
                ...
                if(SmartRouter::handle_input_uri_guard(escape_uri_route) || uri_route == request_uri) {
                    // Change DELETE_METHOD parameter to "true" for tell to Router Service, if routing request method given                
                    Database::set::string({"FUSION_STORE", "FS_ROUTE", "DELETE_METHOD", "is_null"}, "false");

                    // Assign request context to Router Services
                    RouteService::web::assign(escape_uri_route, handler_opt, "DELETE");
                }
            }
        }
    }
